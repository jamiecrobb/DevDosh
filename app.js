    ...
    axios({
        ...
        .then((response) => {
            ...
            for (let i = expenseList.length - 1; i >= 0; i--) {
                ...
                parentList += '<li>' + `(£${expenseList[i].value})` + " " + expenseList[i].name + `<button onclick="removeExpenses(${expenseList[i].id})" class="remove">${svg}</button></li>`;
